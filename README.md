...
**Universality.** A programming language is _universal_ (or Turing complete if you like) if anything that can be computed can be computed using the language. Ruby, Python, Java, C, etc. all possess this property of universality. Hacks have been added on top of YAML to try to make it universal, and these hacks "work", but the cost is ghastly. Why not just use a language that already has universality built in?
